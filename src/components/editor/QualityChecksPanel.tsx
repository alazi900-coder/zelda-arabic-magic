...
function checkGrammar(translation: string): { type: string; message: string } | null {
  ...
  // Each rule: [regex, message]
  const rules: [RegExp, string][] = [
    // تعريف مكرر
    [/الال/, 'تعريف مكرر "الال"'],
    // تاء مربوطة مكررة
    [/ةة/, 'تاء مربوطة مكررة "ةة"'],
    // همزة القطع بدل الوصل في كلمات شائعة
    [/\bإستخدام/, 'همزة خاطئة: "إستخدام" ← "استخدام"'],
    [/\bإستطاع/, 'همزة خاطئة: "إستطاع" ← "استطاع"'],
    [/\bإنتقل/, 'همزة خاطئة: "إنتقل" ← "انتقل"'],
    [/\bإنتظ/, 'همزة خاطئة: "إنتظر/إنتظار" ← "انتظر/انتظار"'],
    [/\bإستمر/, 'همزة خاطئة: "إستمر" ← "استمر"'],
    [/\bإكتشف/, 'همزة خاطئة: "إكتشف" ← "اكتشف"'],
    [/\bإنطلق/, 'همزة خاطئة: "إنطلق" ← "انطلق"'],
    [/\bإحتاج/, 'همزة خاطئة: "إحتاج" ← "احتاج"'],
    [/\bإختر\b/, 'همزة خاطئة: "إختر" ← "اختر"'],
    [/\bإختيار/, 'همزة خاطئة: "إختيار" ← "اختيار"'],
    [/\bإنضم/, 'همزة خاطئة: "إنضم" ← "انضم"'],
    [/\bإستعد/, 'همزة خاطئة: "إستعد" ← "استعد"'],
    [/\bإستلم/, 'همزة خاطئة: "إستلم" ← "استلم"'],
    // ألف مقصورة vs ياء
    [/\bالي\b/, 'ألف مقصورة: "الي" ← "إلى"'],
    [/\bعلي\b(?!\s+(?:بن|ابن|أن))/, 'ألف مقصورة: "علي" ← "على" (إن لم يكن اسم علم)'],
    [/\bحتي\b/, 'ألف مقصورة: "حتي" ← "حتى"'],
    [/\bمتي\b/, 'ألف مقصورة: "متي" ← "متى"'],
    [/\bلدي\b/, 'ألف مقصورة: "لدي" ← "لدى"'],
    [/\bسوي\b/, 'ألف مقصورة: "سوي" ← "سوى"'],
    [/\bأخري\b/, 'ألف مقصورة: "أخري" ← "أخرى"'],
    [/\bكبري\b/, 'ألف مقصورة: "كبري" ← "كبرى"'],
    [/\bصغري\b/, 'ألف مقصورة: "صغري" ← "صغرى"'],
    // تاء مربوطة vs هاء
    [/\bهذة\b/, 'تاء مربوطة: "هذة" ← "هذه"'],
    // خلط التاء المفتوحة والمربوطة في نهاية الكلمات
    [/\bمفتوحت\b/, 'تاء خاطئة: "مفتوحت" ← "مفتوحة"'],
    [/\bمغلقت\b/, 'تاء خاطئة: "مغلقت" ← "مغلقة"'],
    // ال الشمسية مع مسافة
    [/ال\s+[تثدذرزسشصضطظنل]/, 'مسافة بعد ال التعريف قبل حرف شمسي'],
    // واو عطف ملتصقة (مثل "والعب" بدل "و العب" أو "ولعب")
    // كلمة واحدة بأحرف مكررة 3+ (مثل: "اااا")
    [/(.)\1{2,}/, 'حرف مكرر أكثر من مرتين'],
    // نقطتان متتاليتان
    [/\.{2}(?!\.)/, 'نقطتان متتاليتان (استخدم ... أو نقطة واحدة)'],
    // فاصلة إنجليزية بدل عربية
    [/[\u0600-\u06FF],[\u0600-\u06FF]/, 'فاصلة إنجليزية بين كلمات عربية (استخدم ، العربية)'],
    // علامة استفهام إنجليزية مع نص عربي
    [/[\u0600-\u06FF]\s*\?/, 'علامة استفهام إنجليزية مع نص عربي (استخدم ؟)'],
    // فاصلة منقوطة إنجليزية
    [/[\u0600-\u06FF]\s*;/, 'فاصلة منقوطة إنجليزية (استخدم ؛ العربية)'],
    // "اللاعب" خاطئة → "الاعب"
    [/\bالاعب\b/, 'إملاء: "الاعب" ← "اللاعب"'],
    // مسافة قبل علامة ترقيم
    [/\s[،؟؛!]/, 'مسافة زائدة قبل علامة ترقيم'],
    // "شيئ" بدل "شيء"
    [/شيئ\b/, 'إملاء: "شيئ" ← "شيء"'],
    // "مسئول" vs "مسؤول"
    [/مسئول/, 'إملاء: "مسئول" ← "مسؤول"'],
    // "إنشاء الله" → "إن شاء الله"
    [/إنشاء\s*الله/, 'إملاء: "إنشاء الله" ← "إن شاء الله"'],
    // "ان" بدون همزة في بداية الجملة
    [/(?:^|[.،؟!]\s*)ان\s/, '"ان" بدون همزة (إن/أن)'],
  ];

  for (const [regex, message] of rules) {
    if (regex.test(stripped)) {
      return { type: "grammar_check", message: `📝 ${message}` };
    }
  }
  return null;
}
